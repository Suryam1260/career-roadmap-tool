...
const AccordionTrigger = React.forwardRef(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex flex-1 items-center justify-between py-4 px-5 font-semibold text-base text-slate-900 transition-all hover:bg-slate-50 [&[data-state=open]>svg]:rotate-180 rounded-none",
        className
      )}
      {...props}
    >
